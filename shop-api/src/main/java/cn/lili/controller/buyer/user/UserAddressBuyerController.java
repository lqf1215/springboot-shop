...
public class UserAddressBuyerController {
    ...
    public ResultMessage<IPage<UserAddress>> page(PageVO page) {
        return ResultUtil.data(userAddressService.getAddressByMember(page, 23));
    }
    ...
    public ResultMessage<UserAddress> addShippingAddress(@Valid UserAddress shippingAddress) {
        //添加会员地址
//        shippingAddress.setUserId(Objects.requireNonNull(UserContext.getCurrentUser()).getId());
        shippingAddress.setUserId(23);
    ...
    public ResultMessage<Object> delShippingAddressById(@PathVariable String id) {
        OperationalJudgment.judgment(userAddressService.getById(id));
