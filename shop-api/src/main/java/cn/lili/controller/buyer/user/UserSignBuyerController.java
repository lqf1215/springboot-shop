...
public class UserSignBuyerController {
    @Autowired
    ...
    public ResultMessage<Boolean> memberSign() {
        return ResultUtil.data(userSignService.memberSign());
    }

    @GetMapping
    @ApiOperation(value = "根据时间查询会员签到表，类型是YYYYmm")
    public ResultMessage<List<UserSign>> memberSign(String time) {
        return ResultUtil.data(userSignService.getMonthSignDay(time));
    }
