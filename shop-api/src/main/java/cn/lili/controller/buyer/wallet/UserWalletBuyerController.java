...
public class UserWalletBuyerController {
    ...
    public ResultMessage<MemberWalletVO> get() {
        ...
        if (authUser != null) {
            return ResultUtil.data(userWalletService.getMemberWallet(authUser.getId()));
        }
    ...
    public ResultMessage<Object> setPassword(String password, @RequestHeader String uuid) {
        ...
        if (verificationService.check(uuid, VerificationEnums.WALLET_PASSWORD)) {
            userWalletService.setMemberWalletPassword(user, password);
    ...
    public ResultMessage updatePassword(@RequestParam @Pattern(regexp = "[a-fA-F0-9]{32}", message = "旧密码格式不正确") String oldPassword,
        ...
        UserWallet userWallet = this.userWalletService.getOne(new QueryWrapper<UserWallet>().eq("user_id", user.getId()));
        //校验新旧密码是否一致
        if (userWallet != null) {
//            if (!new BCryptPasswordEncoder().matches(oldPassword, userWallet.getWalletPassword())) {
//                throw new ServiceException(ResultCode.USER_OLD_PASSWORD_ERROR);
//            }
            this.userWalletService.setMemberWalletPassword(user, newPassword);
    ...
    public Boolean checkPassword() {
        return userWalletService.checkPassword();
    }
    ...
    public ResultMessage<Boolean> withdrawal(@Max(value = 9999, message = "充值金额单次最多允许提现9999元") @Min(value = 1, message = "充值金额单次最少提现金额为1元") Double price) {
        return ResultUtil.data(userWalletService.applyWithdrawal(price));
    }
