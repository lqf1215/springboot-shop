...
public class MemberPointExecute implements MemberRegisterEvent, GoodsCommentCompleteEvent, OrderStatusChangeEvent, AfterSaleStatusChangeEvent {
    ...
    public void goodsComment(UserEvaluation userEvaluation) {
        ...
        PointSetting pointSetting = getPointSetting();
        //赠送会员积分
        userService.updateMemberPoint(pointSetting.getComment().longValue(), PointTypeEnum.INCREASE.name(), userEvaluation.getUserId(), "会员评价，赠送积分" + pointSetting.getComment() + "分");
    }
    ...
    public void afterSaleStatusChange(AfterSale afterSale) {
        if (afterSale.getServiceStatus().equals(AfterSaleStatusEnum.COMPLETE.name())) {
            ...
            //扣除会员积分
            userService.updateMemberPoint(point.longValue(), PointTypeEnum.REDUCE.name(), afterSale.getUserId(), "会员退款，回退积分" + point + "分");
