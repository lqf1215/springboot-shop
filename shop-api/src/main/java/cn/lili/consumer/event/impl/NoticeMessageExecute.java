...
public class NoticeMessageExecute implements TradeEvent, OrderStatusChangeEvent, AfterSaleStatusChangeEvent, MemberPointChangeEvent, MemberWithdrawalEvent {
    ...
    public void orderCreate(TradeDTO tradeDTO) {
        ...
        noticeMessageDTO.setUserId(tradeDTO.getUserId());
    ...
    public void afterSaleStatusChange(AfterSale afterSale) {
        NoticeMessageDTO noticeMessageDTO = new NoticeMessageDTO();
        noticeMessageDTO.setUserId(afterSale.getUserId());
        Map<String, String> params = new HashMap<>(2);
    ...
    @Override
    public void memberPointChange(UserPointMessage userPointMessage) {
        if (userPointMessage == null) {
            return;
        }
        //组织站内信参数
        NoticeMessageDTO noticeMessageDTO = new NoticeMessageDTO();
        noticeMessageDTO.setUserId(userPointMessage.getUserId());
        Map<String, String> params = new HashMap<>(2);
        if (userPointMessage.getType().equals(PointTypeEnum.INCREASE.name())) {
            params.put("expenditure_points", "0");
            params.put("income_points", userPointMessage.getPoint().toString());
        } else {
            params.put("expenditure_points", userPointMessage.getPoint().toString());
    ...
    public void memberWithdrawal(MemberWithdrawalMessage memberWithdrawalMessage) {
        NoticeMessageDTO noticeMessageDTO = new NoticeMessageDTO();
        noticeMessageDTO.setUserId(memberWithdrawalMessage.getUserId());
