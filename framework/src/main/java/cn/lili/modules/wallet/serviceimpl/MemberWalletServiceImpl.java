...
public class MemberWalletServiceImpl extends ServiceImpl<MemberWalletMapper, MemberWallet> implements MemberWalletService {
    ...
    @Override
    public MemberWalletVO getMemberWallet(String userId) {
        //构建查询条件
        ...
        if (memberWallet == null) {
            memberWallet = this.save(userId, userService.getById(userId).getName());
        }
        //返回查询数据
    ...
    public Boolean increaseWithdrawal(MemberWalletUpdateDTO memberWalletUpdateDTO) {
        ...
        MemberWallet memberWallet = this.checkMemberWallet(memberWalletUpdateDTO.getUserId());
        //余额变动
        ...
        //新增预存款日志
        WalletLog walletLog = new WalletLog(memberWallet.getUserName(), memberWalletUpdateDTO);
    ...
    public Boolean reduce(MemberWalletUpdateDTO memberWalletUpdateDTO) {
        ...
        this.updateById(memberWallet);
        //新增预存款日志
        WalletLog walletLog = new WalletLog(memberWallet.getUserName(), memberWalletUpdateDTO, true);
        walletLogService.save(walletLog);
        return true;
    }
     ...
     * @param userId 会员id
     */
    private MemberWallet checkMemberWallet(String userId) {
        //获取会员预存款信息
        MemberWallet memberWallet = this.getOne(new QueryWrapper<MemberWallet>().eq("user_id", userId));
        //如果会员预存款信息不存在则同步重新建立预存款信息
        if (memberWallet == null) {
            User user = userService.getById(userId);
            if (user != null) {
                memberWallet = this.save(userId, user.getName());
            } else {
    ...
    @Override
    public MemberWallet save(String userId, String userName) {
        //获取会员预存款信息
        MemberWallet memberWallet = this.getOne(new QueryWrapper<MemberWallet>().eq("user_id", userId));
        if (memberWallet != null) {
            return memberWallet;
        ...
        memberWallet.setUserId(userId);
        memberWallet.setUserName(userName);
    ...
    public Boolean applyWithdrawal(Double price) {
        ...
        memberWithdrawApply.setUserId(authUser.getId());
        memberWithdrawApply.setUserName(authUser.getNickName());
        ...
        if (setting != null) {
            ...
            if (Boolean.FALSE.equals(withdrawalSetting.getApply())) {
                ...
                MemberWalletVO memberWalletVO = this.getMemberWallet(memberWithdrawApply.getUserId());
