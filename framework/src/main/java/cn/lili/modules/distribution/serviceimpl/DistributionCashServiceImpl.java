...
public class DistributionCashServiceImpl extends ServiceImpl<DistributionCashMapper, DistributionCash> implements DistributionCashService {
    ...
    public Boolean cash(Double applyMoney) {
        ...
        if (distribution != null && distribution.getDistributionStatus().equals(DistributionStatusEnum.PASS.name())) {
            ...
            //提现申请记录
            DistributionCash distributionCash = new DistributionCash("D" + SnowFlake.getId(), distribution.getId(), applyMoney, distribution.getUserName());
            boolean result = this.save(distributionCash);
            if (result) {
                //发送提现消息
                MemberWithdrawalMessage memberWithdrawalMessage = new MemberWithdrawalMessage();
                memberWithdrawalMessage.setUserId(distribution.getUserId());
        ...
        if (distributorCash != null) {
            ...
            if (distribution != null && distribution.getDistributionStatus().equals(DistributionStatusEnum.PASS.name())) {
                ...
                if (result.equals(WithdrawStatusEnum.VIA_AUDITING.name())) {
                    ...
                    //提现到余额
                    memberWalletService.increase(new MemberWalletUpdateDTO(distributorCash.getPrice(), distribution.getUserId(), "分销[" + distributorCash.getSn() + "]佣金提现到余额[" + distributorCash.getPrice() + "]", DepositServiceTypeEnum.WALLET_COMMISSION.name()));
                } else {
